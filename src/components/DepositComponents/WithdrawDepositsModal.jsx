import {
  ...
  import { AddSavingsSchema, WithdrawSavingsSchema } from "../../utils/validationSchema";
  import {
    useAddMemberDepositsMutation,
    useGetDepositsAccountsQuery,
    useWithdrawMemberDepositsMutation,
  } from "../../services/deposits/depositSlice";
  
  const WithdrawDepositsModal = ({ open, onClose }) => {
    ...
    const [withdrawMemberDeposits, { isLoading }] = useWithdrawMemberDepositsMutation();
    ...
    const onSubmitHandler = async (data, event) => {
      ...
      console.log(data)
      try {
        const response = await withdrawMemberDeposits({
    ...
    return (
      <Dialog open={open} onClose={onClose} maxWidth="md">
        <DialogTitle>Withdraw Deposits</DialogTitle>
        <DialogContent>
          <Box
            mt="20px"
            display="grid"
  ...
  export default WithdrawDepositsModal;
