...
const RejectLoan = () => {
  ...
  const handleUpdateStatus = async (loanId) => {
    try {
      const deniedLoan = status?.results.find(
        (s) => s.status_name === "DENIED"
      );
      if (deniedLoan) {
        const textNotify = watch("text");
        const response = await updateLoan({
          loanId,
          data: { id: deniedLoan.id, text: textNotify }, // Update data property to send object with id key
        });
      ...
      >
        ...
        {selectedRow ? (
          ...
          >
            ...
            <RHFCheckbox
              disabled={selectedRow?.status_name === "DENIED"}
              ...
              padding={3}
            />
            {selectedRow?.status_name == "DENIED" ? (
              <Button
                type="submit"
